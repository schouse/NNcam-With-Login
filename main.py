...
if(Login_System_Needed):
    ...
    # install wkhtmltopdf which deals with images and other complex things
    # wkhtmltopdf is open source command line tools to render HTML into PDF and various image formats
    # wkhtmltopdf = 'C:\\Program Files\\wkhtmltopdf\\bin\\wkhtmltopdf.exe'
    path_wkthmltopdf = 'C:\\Program Files\\wkhtmltopdf\\bin\\wkhtmltopdf.exe'
    path = Path(path_wkthmltopdf)
    if path.is_file():
        print("wkhtml found")
        config = pdfkit.configuration(wkhtmltopdf=path_wkthmltopdf)
        # config = pdfkit.configuration(wkhtmltopdf='C:\\Program Files\\wkhtmltopdf\\bin\\wkhtmltopdf.exe')
    else:
        print("wkhtml not found")
        # QMessageBox.warning(self, 'WKPDF module not found')

    #==================================== Defining dll path for DSC ==================================
    if sys.platform == 'win32': # Check if OS is Windows. sys.platform will be win32 regardless of the bitness
        # 'r' at the start of the pattern string designates a python "raw" string which passes through backslashes without change
        dllpath =[ r'c:\windows\System32\eps2003csp11.dll',  r'c:\windows\System32\mTokenMiniDrv.x64.dll', r'c:\windows\System32\SignatureP11.dll'  ]  # Probably need to manually instal these
    else: # Check if Linux
    ...
    def writewithname(username, pdf, outputpdf):
        with open(pdf, 'rb') as file:
            ...
            pdf_writer.encrypt(user_password='', owner_pwd='123456', permissions_flag=0b0100)
    ...
    class AdminPanel(QtWidgets.QDialog): # This class defined with methods that are being used to execute operations which an admin can do
        ...
        def showUsers(self): # Show user table in admin panel
            ...
            self.tableWidget.setColumnCount(4)   # There are 4 columns in Login List
        ...
        def showLoginRecords(self): # Show login record table in admin panel
            ...
            self.tableWidget.setColumnCount(5) # There are 5 columns in Login List
    ...
    def initCamera(self):
        ...
        if len(a) <= 0 or a == None: # If there are no cameras then 'a' will be 0
            ...
            try:
                ...
                self.hcam.put_Option(nncam.NNCAM_OPTION_BITDEPTH, 0)        # 0 use 8-bits, 1 use max bits
                self.hcam.put_Option(nncam.NNCAM_OPTION_RAW, 0)             # 0 RGB mode, 1 RAW mode
                self.hcam.put_Option(nncam.NNCAM_OPTION_RGB, 0)             # 0 = RGB24 (basically 8-bits
                                                                            # 1 = enable RGB48 format when bitdepth > 8
                                                                            # 2 = RGB32
                                                                            # 3 = 8 bits gray (only for mono)
                                                                            # 4 = 16 bits gray (only for mono and bitdepth > 8)
    ...
    def eventImageSignal(self): # This sends a video signal (LIVE IMAGE)
        ...
        if self.hcam is not None:
            try:
                ...
                # self.hcam.PullImageV2(self.buf, 16, None)  # For 16 bit
                self.hcam.PullImageV2(self.buf, 8, None)
                ...
                if self.ui.radioChemiMode.isChecked() & self.flagToSaveLongExpoImage is True:
                    ...
                    self.saveBufferToImage(self.buf, wd, ht)  # Chemi Mode

                # videoFromRawBuffer = np.frombuffer(self.buf, dtype="uint16") # Create numpy array of 16-bit image data pulled earlier
                videoFromRawBuffer = np.frombuffer(self.buf, dtype="uint8") # Create numpy array of 8-bit image data pulled earlier
                ...
                print("Image acquisition failed12") # TODO: Happens when image rotated. So for rotation, stop image display just before rotation
            else:
                ...
                self.liveImage = QImage(videoFromRawBuffer, self.imgWidth, self.imgHeight, (self.imgWidth * 1), QImage.Format_Grayscale8)
                # Liveimage for RGB
...
# For MONO Images
    ...
    def changeRotation(self):
        ...
        if self.hcam is not None:
            ...
            # FOR MONO CAMERAS 8 bit:
            self.pic = bytes(self.imgWidth * self.imgHeight)
            self.buf = bytes((self.imgWidth) * self.imgHeight) # This is on page 7 of manual
            print("Buffer Size:", len(self.buf))
